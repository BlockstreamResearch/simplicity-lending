 ...
 *
 * Called on a product screen through `useLenderVaultClaim` below, and directly by a wallet that
 * performs this action for itself, which cannot use a hook to reach the facade.
 */
export function lenderVaultClaimBuilder(access: ProtocolBuilderAccess) {
  const {
    lwkNetwork,
    getReceiveAddress,
