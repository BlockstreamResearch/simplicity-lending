...
export interface CreateOfferParams {
  ...
  /**
   * The height the loan is liquidatable at, when the caller already knows it.
   *
   * A deployment states a height rather than a term, so an action performed from a document
   * carries the height itself. Without one the term is counted from the tip this reads, which is
   * what a person choosing "30 days" on a form means.
   */
 ...
 *
 * Called on a product screen through `useCreateOffer` below, and directly by a wallet that
 * performs this action for itself, which cannot use a hook to reach the facade.
 */
export function createOfferBuilder(access: ProtocolBuilderAccess) {
  const {
    lwkNetwork,
    getReceiveAddress,
  ...
  const createOffer = async (
    ...
    const offerParameters = {
      ...
      loanExpirationTime: toUint32(
        params.loanExpirationTime ?? currentBlockHeight + loanDurationBlocks,
        'loanExpirationTime',
      ),
    }
