...
/** OP_RETURN "burn" script (6a 04 6275726e). */
...
function requireVout(tx: EsploraTx, vout: number, label: string): EsploraVout {
  ...
  if (!out) throw new Error(`${label} vout ${vout} missing in lending tx`)
  return out
}
...
export async function buildLoanLiquidationTx(
  ...
  const lendingPrevout = requireVout(lendingTx, 0, 'Lending')
  const firstParamsPrevout = requireVout(lendingTx, 2, 'First parameters NFT')
  const secondParamsPrevout = requireVout(lendingTx, 3, 'Second parameters NFT')
  ...
  const borrowerNftPrevout = requireVout(lendingTx, 4, 'Borrower NFT')
