...
const STORAGE_PREFIX = 'simplicity-lending-lender-offers'
...
export function loadTrackedLenderOfferIds(
...
export function rememberTrackedLenderOfferId(
  signingXOnlyPubkey: string,
  network: WalletAbiNetwork,
  offerId: string
): string[] {
  ...
  if (normalizedOfferId.length === 0) {
    return loadTrackedLenderOfferIds(signingXOnlyPubkey, network)
  }

  const nextOfferIds = normalizeOfferIds([
    normalizedOfferId,
    ...loadTrackedLenderOfferIds(signingXOnlyPubkey, network),
  ])
