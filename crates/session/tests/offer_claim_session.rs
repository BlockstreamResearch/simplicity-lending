...
use utils::{
    ...
    claim_borrower_principal, create_active_factory, dummy_principal_asset_id, fund_asset_outputs,
    issue_asset, liquidate_active_offer, offer_params, repay_active_offer, setup_it_context_pool,
...
#[tokio::test]
#[serial]
async fn claim_principal_fails_on_cancelled_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
...
#[tokio::test]
#[serial]
async fn claim_principal_fails_on_repaid_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
...
#[tokio::test]
#[serial]
async fn claim_principal_fails_on_liquidated_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
...
#[tokio::test]
#[serial]
async fn claim_principal_fails_when_already_claimed() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
...
#[tokio::test]
#[serial]
async fn lender_cannot_claim_principal_on_active_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
    ...
    let result = lender.claim_principal(&offer_id.to_string()).await;
