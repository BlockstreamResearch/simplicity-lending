...
use utils::{
    ...
    claim_lender_vault, create_active_factory, create_and_broadcast_offer,
    dummy_principal_asset_id, fund_asset_outputs, issue_asset, liquidate_active_offer,
    offer_params, repay_active_offer, seed_pending_offer, setup_it_context_pool,
...
#[tokio::test]
#[serial]
async fn cancel_offer_fails_on_repaid_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
...
#[tokio::test]
#[serial]
async fn cancel_offer_fails_on_claimed_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
...
#[tokio::test]
#[serial]
async fn cancel_offer_fails_on_liquidated_offer() -> anyhow::Result<()> {
    let (context, pool) = setup_it_context_pool().await?;
    let (indexer_url, server_handle) = start_indexer_api(pool.clone()).await?;
    let borrower = build_session(&context, &indexer_url);
    let lender = build_session_with_signer(&context, context.random_signer(), &indexer_url);
