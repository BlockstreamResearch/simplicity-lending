...
use crate::sdk::lending::basic::{add_base_input_from_utxo, check_asset_id, check_asset_value};
...
pub fn build_lending_creation(
    ...
    principal_output_script: &Script,
    ...
    check_asset_id(collateral_asset_id, lending_arguments.collateral_asset_id())?;
    ...
    let lending_auth_taproot_pubkey_gen = TaprootPubkeyGen::from(
        &ScriptAuthArguments {
    ...
    let parameters_nft_output_script = lending_auth_taproot_pubkey_gen.address.script_pubkey();

    let mut pst = PartiallySignedTransaction::new_v2();

    // Inputs setup

    // Add PreLock Covenant input with the collateral asset
    ...
    pst.add_output(Output::new_explicit(
        lending_taproot_pubkey_gen.address.script_pubkey(),
        collateral_value,
        collateral_asset_id,
        None,
    ));

    // Add Principal amount output
    pst.add_output(Output::new_explicit(
        principal_output_script.clone(),
        principal_value,
        principal_asset_id,
        None,
    ));
    ...
    pst.add_output(Output::new_explicit(
        parameters_nft_output_script.clone(),
    ...
    pst.add_output(Output::new_explicit(
        borrower_nft_output_script.clone(),
        borrower_nft_value,
        borrower_nft_asset_id,
        None,
    ));
