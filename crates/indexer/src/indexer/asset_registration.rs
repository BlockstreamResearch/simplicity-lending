//! ELIP-0100 asset metadata for factory assets issued by the lending flow.
...
impl AssetRegistration {
    ...
    /// The contract the lending flow commits for a factory asset funded by `prevout`.
    ///
    /// Must stay in sync with `buildAssetContract` in `web/src/hooks/useBorrowerAccount.ts`.
    fn expected_contract(&self, prevout: &OutPoint) -> serde_json::Value {
        json!({
            "entity": { "domain": self.domain },
            "issuer_pubkey": ISSUER_PUBKEY,
            "name": format!("simplicity-lending/v1 {}:{}", prevout.txid, prevout.vout),
            "precision": 0,
            // Max 7 chars: hardware wallets hold tickers in 8-byte buffers
            // incl. NUL (e.g. Jade's sign_tx.c ticker[8]).
            "ticker": format!("SLF{}", &prevout.txid.to_string()[..4]),
            "version": 0,
        })
    }
    ...
    pub fn verified_contract(
        ...
        tx.input
            ...
            .find_map(|input| {
                ...
                let contract = self.expected_contract(&prevout);
                let contract_hash = match ContractHash::from_json_contract(&contract.to_string()) {
                    Ok(hash) => hash,
                    Err(error) => {
                        tracing::warn!(%factory_asset_id, %prevout, %error, "expected contract failed to hash");
                        return None;
                    }
                };
                let computed_asset_id = AssetId::new_issuance(prevout, contract_hash);
                if computed_asset_id == factory_asset_id {
                    tracing::info!(
                        %factory_asset_id, %prevout, %contract_hash,
                        "factory issuance commits the expected asset contract"
                    );
                    Some(contract)
                } else {
                    tracing::warn!(
                        %factory_asset_id, %computed_asset_id, %prevout, %contract_hash,
                        expected_contract = %contract,
                        "factory issuance does not commit the expected asset contract; not registering"
                    );
                    None
                }
            })
    ...
    pub fn spawn_registration(&self, factory_asset_id: AssetId, contract: serde_json::Value) {
        ...
        let asset_id = factory_asset_id.to_string();
...
mod tests {
    ...
    /// Pins the canonical serialization the web flow must reproduce
    /// (`buildAssetContract` in web/src/hooks/useBorrowerAccount.ts).
    #[test]
    fn expected_contract_is_deterministic() {
        let contract = registration("lending.example").expected_contract(&prevout());
        ...
        assert_eq!(
            ...
            concat!(
                ...
                "\"precision\":0,\"ticker\":\"SLF0e19\",\"version\":0}"
            )
        );
    }
    ...
    fn expected_contract_matches_live_issuance() {
        ...
        let contract = registration.expected_contract(&prevout);
